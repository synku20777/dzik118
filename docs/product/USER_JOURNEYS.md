...
- Invoice template configuration: Configure the organization's invoice layout under `/admin/o/:orgId/settings/invoice-template` (reorder sections via drag-and-drop or Move up/down buttons, toggle visibility on the optional Default note/Footer sections, add custom text blocks, configure row formatting/spacing, edit English/Russian translations of header/footer/payment instructions/default note/custom text via the document-language selector, and inspect via a live preview that reflects current tariffs automatically). The four mandatory sections (Invoice details, Sender and recipient, Charges table, Payment details) always appear and have no visibility toggle. Saving updates the organization template for future invoice generation; already-prepared and sent invoices retain their frozen template snapshots, in every language.
- Viewing an invoice in another language: on the admin invoice detail page, the resident portal invoice page, or a public invoice link, use the document-language selector to view or download the invoice as an English or Russian translated copy. This never changes the invoice's amount, number, or status -- Latvian remains the canonical stored document, and a missing translation falls back to Latvian rather than showing blank content.
