...
// (spec Section 16 step 4/5, Section 13.10). Status itself stays
// MISSING_DATA here -- the DRAFT transition only happens when Phase F
// actually generates an invoice (spec Section 19: "inputs complete + invoice
// generated" is one combined step, not two).
...
export async function recalculateCaseReadiness(
  ...
  await tx
    .update(billingCases)
    .set({ missingData, statusUpdatedAt: new Date() })
